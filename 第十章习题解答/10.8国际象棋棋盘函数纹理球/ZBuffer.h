...
public:
	CZBuffer();
    virtual ~CZBuffer();
	void SetPoint(CPi3 *,CVector *,double *,double *,int m);
	void CreateBucket();//创建桶
	void CreateEdge();//边表
	void AddEt(CAET *);//合并ET表
	void EtOrder();//ET表排序
	void Fill(CDC *,CP3,CLighting *,CMaterial *);//填充
	double Interpolation(double,double,double,double,double);//双线性插值
    CVector Interpolation(double,double,double,CVector,CVector);//法矢线性插值
	CRGB GetTextureColor(double,double);//通过纹理获得颜色
protected:
	int PNum;//顶点的个数
	CPi3 *P;//顶点动态数组
	double* u;//纹理动态数组
