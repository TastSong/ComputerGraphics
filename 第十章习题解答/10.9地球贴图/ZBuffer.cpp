...
void CZBuffer::Fill(CDC *pDC,COLORREF **Image,CP3 ViewPoint,CLighting *pLight,CMaterial *pMaterial)//填充面片
{
	...
	for(CurrentB=HeadB;CurrentB!=NULL;CurrentB=CurrentB->next)
	{
		...
		for(T1=HeadE;T1!=NULL;T1=T1->next)
		{
			...
			else
			{
				...
				{
					uf=((Interpolation(x,xb,xe,ua,ub)>511)?511:(Interpolation(x,xb,xe,ua,ub)));
					vf=((Interpolation(x,xb,xe,va,vb)>255)?255:(Interpolation(x,xb,xe,va,vb)));
					nf=Interpolation(x,xb,xe,na,nb);
					CP3 p(ROUND(x),CurrentB->ScanLine,z);   
					COLORREF Imageclr=Image[int(uf)][ROUND(vf)];
					pDC->SetPixel(ROUND(p.x),ROUND(p.y),Imageclr);
					z+=zStep;
				}
				Flag=FALSE;
			}
